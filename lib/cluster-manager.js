...
module.exports = class ClusterManager {
  ...
  static fork (queueName) {
    ...
    return co(function * () {
      ...
      debug(`worker created "${queueName}:${queueWorkerNum}"`)
    }).catch(function (err) {
  ...
  static stopWorker (worker) {
    ...
    return co(function * () {
      // donot respawn worker on exit
      worker.removeListener('exit', ClusterManager.respawnWorker)
      // exit worker
      // TODO: send accepts a callback
      worker.send('shutdown') // handled in application.js
  ...
  start () {
    ...
    this.startingPromise = co(function * () {
      ...
      if (numCpus > numQueues) {
        ...
        let numClones = Math.ceil(numCpus / numQueues)

        queueNames = []
        times(numClones, function () {
