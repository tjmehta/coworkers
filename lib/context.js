...
const Context = module.exports = class Context {
  ...
  toJSON () {
    ...
    let json
    try {
      json = pick(this, ctxKeys)
    } catch (err) {
      if (err.message !== 'Ack shortcuts are not available in the errorHandler, please use "consumerChannel" methods directly') {
        throw err
      }
      json = pick(this, remove(ctxKeys, acks))
    }
    return json
  }
}
