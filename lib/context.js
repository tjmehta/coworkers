...
var castToBuffer = require('./utils/cast-to-buffer.js')
var callbackOrPromise = require('./utils/cb-or-promise.js')
...
module.exports = class Context {
  constructor (app, queueName, message) {
    ...
    this.onerror = app.onerror.bind(this)
    this.finalhandler = app.finalhandler.bind(this)
    this.queueName = queueName
    this.message = message
    this.state = {}
    // ack, nack, ackAll, nackAll
    this._channelAction = {}
    ...
    function channelActionProperty (method, argNames) {
      return {
        get () {
          return this._channelAction.method === method
            ? this._channelAction.args
            : undefined
        },
        set (value) {
          if (!value) {
            if (this._channelAction.method === method) {
              this._channelAction = {}
            }
          ...
          this._channelAction.method = method
          this._channelAction.args = pick(value, argNames)
  ...
  reply (content, options) {
    ...
    content = castToBuffer(args.content)
    options = args.options
    // set correlation id for the reply message
    options.correlationId = correlationId
    // reply on the message origin channel: consumerChannel
    return this.consumerChannel.sendToQueue(replyTo, content, options)
  }
  /**
   ...
   * @param  {Object}   [queueOpts] assertQueue options for replyTo queue
   * @param  {Object}   [consumeOpts] consume options for replyTo queue
   * @param  {Function} [cb]        callback
   * @return {Promise|null}         returns a promise if callback is not provided
   */
  request (queue, content, sendOpts, queueOpts, consumeOpts, cb) {
    const self = this
    const args = assertArgs(arguments, {
      'queue': 'string',
    ...
    return callbackOrPromise(promise, cb)
  }
}
