...
Object.assign(Application.prototype, {
  ...
  /**
   * Setup prefetch options for consumer channel (`consumerChannel`)
   * Equivalent to http://www.squaremobius.net/amqp.node/channel_api.html#channel_prefetch
  ...
  connect (url, socketOpts, cb) {
    ...
    this.connectingPromise = co(function * () {
      ...
      if (!self.consumerChannel) {
        // app.consumerChannel
        yield createAppChannel(self, 'consumerChannel') // uses: self.connection, self.prefetchOpts
      }
  ...
  close (cb) {
    ...
    this.closingPromise = co(function * () {
      ...
      if (self.publisherChannel) {
        ...
        let channel = self.publisherChannel
        channel.removeListener('close', channel.__coworkersCloseHandler)
        channel.removeListener('error', channel.__coworkersErrorHandler)
        yield channel.close()
      }
